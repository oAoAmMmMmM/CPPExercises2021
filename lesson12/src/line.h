...
public:
    ...
    void plot(cv::Mat &img, double scale, const cv::Scalar& color); // метод прямой позволяющий нарисовать ее на картинке (т.е. на простом графике)

    double getYFromX(double x); // метод прямой позволяющий рассчитать точку на прямой (найти y по x)

    double getDistanceSqr(const cv::Point2f&) const;
